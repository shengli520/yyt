...
class BasePage():
    ...
    def switch_frame(self, selector):
        logger.info("Into the frame is % iframe ***")
        return self.driver.switch_to.frame(selector)

    def back_frame(self):
        logger.info("back frame")
    ...
    def get_txt(self, text):
        txt = self.find_element(text).text
        return txt

    # #Select方法 重写
    # def get_select(self,selector,text):
    #     return self.(find_element(selector)).select_by_value(text)
    #     logger.info("The element \' %s \' was selected." % text)
